...
export default function Home() {
  ...
  useEffect(() => {
    if (isReady && welcomeScreenVisible && Capacitor.isNativePlatform()) {
        const hasSeenWelcome = localStorage.getItem('hasSeenWelcomePopup');
        if (!hasSeenWelcome) {
            setIsWelcomeOpen(true);
        }
    }
  }, [isReady, welcomeScreenVisible]);

  const handleWelcomeConfirm = () => {
    localStorage.setItem('hasSeenWelcomePopup', 'true');
    setIsWelcomeOpen(false);
    // Suggest enabling notifications, but don't force it.
    // The user can do it from settings.
    ...
    <div className="flex flex-col min-h-screen bg-background">
      ...
      <main className="flex-1 flex flex-col items-center justify-center">
        <div className="container mx-auto w-full max-w-md flex-1 py-8">
          ...
          {isReady && welcomeScreenVisible && <WelcomeScreen />}
          {isReady && dashboardVisible && <PrepDashboard 
                                prises={prises}
                                addDose={addDose}
                                ...
                                welcomeScreenVisible={welcomeScreenVisible}
